<?php namespace etiqa\Oldstyle\Command;
    ...
    protected function configure()
    {
        $this
            ...
            ->addOption(
                ...
                'When specified, all packages defined in the .Oldstyle config will be installed.'
            )
            ->addOption(
                'interactive',
                'i',
                InputOption::VALUE_NONE,
                'When --all and --interactive are specified, all packages defined in the .Oldstyle config will be installed interactively.'
            );
    }
        ...
        if ($input->getOption('all')) {
            ...
            foreach ($packages as $provider_name => $provider_data) {
                ...
                if (!$provider) {
                    ...
                    // Try to look for a file with the API Key from a file within the Oldstyle working directory
                    if (!empty($provider_data['api_key']) && file_exists(OLDSTYLE_WORKING_DIR . '/' . $provider_data['api_key'])) {
                        $api_key = trim(file_get_contents(OLDSTYLE_WORKING_DIR . '/' .$provider_data['api_key']));
                    }
